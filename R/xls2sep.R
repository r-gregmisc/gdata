                    ...
                    perl="perl")
{
  xls2sep(xls=xls, sheet=sheet, verbose=verbose,
          blank.lines.skip=blank.lines.skip, ..., method="csv", perl=perl)
}
...
{
  xls2sep(xls=xls, sheet=sheet, verbose=verbose,
          blank.lines.skip=blank.lines.skip, ..., method="tab", perl=perl)
}
...
{
  xls2sep(xls=xls, sheet=sheet, verbose=verbose,
          blank.lines.skip=blank.lines.skip, ..., method="tsv", perl=perl)
}

xls2sep <- function(xls, sheet=1, verbose=FALSE, blank.lines.skip=TRUE, ...,
                    method=c("csv","tsv","tab"), perl = perl)
{
  ...
  cmd <- paste(shQuote(perl), shQuote(script),
               skipBlank,  # flag is not quoted
               shQuote(xls), shQuote(targetFile), shQuote(sheet))
